...
import {
  ...
  isCollaborationTool,
} from "../canvas/collaboration-tools.js";
import {
  CONFIG_DIR,
  DEFAULT_PORT,
  ensureSiteWorkspace,
...
import {
  ...
  resolveComfyTask,
  type ComfyUiClient,
} from "../runtime/comfy-client.js";
import {
  loadPluginMcpDeclarationsFromBackend,
  savePluginMcpDeclarationsToBackend,
  type PluginMcpDeclaration,
} from "./plugin-mcp.js";
...
export function createAgentApp(options: AgentHttpOptions = {}) {
  ...
  const emit = (type: string, payload: unknown) => {
    ...
    if (type === "agent_bootstrap" && value.phase === "preheat") {
      if (value.type === "mcp.startup")
        session.updateConversationMcp(
          String(value.name || ""),
          startupStatus(value.status),
          String(value.error || "") || null,
          String(value.failureReason || "") || null,
        );
      if (value.type === "mcp.complete")
        session.completeConversationMcpInventory(mcpInventory(value.services));
    }
    const scope = session.codexBusy
      ? session.codexEventScope
      : { threadId: "", turnId: "", sourceClientId: "" };
    const threadId = String(
      value.threadId ||
        value.thread_id ||
        scope.threadId ||
        ensureSiteWorkspace(config).activeThreadId ||
        "",
    );
    const turnId = String(value.turnId || value.turn_id || scope.turnId || "");
    const sourceClientId = String(
      value.sourceClientId || scope.sourceClientId || "",
    );
    const data = {
      ...value,
  ...
  const setActiveThread = (
    ...
    const workspace = updateSiteWorkspace(config, {
      activeThreadId: activeThreadId || undefined,
    });
    if (!preserveConversation)
      session.activateConversation(
        activeThreadId,
        String(payload.sourceClientId || "") || undefined,
      );
    if (!session.codexBusy && session.codexThreadId !== activeThreadId)
      session.setCodexState({ threadId: activeThreadId, turnId: "" });
    ...
    prepared = (async () => {
      ...
      try {
        ...
        if (threadId && !ensureSiteWorkspace(config).activeThreadId) {
          ...
          setActiveThread(
            threadId,
            { emptyThread: true, draftThread: true, sourceClientId: clientId },
            true,
          );
        }
        return thread;
  ...
  const prepareExistingThread = async (
    threadId: string,
    clientId = "",
    permission: AgentPermissionMode = "request",
  ) => {
    const workspace = ensureSiteWorkspace(config);
  ...
  const failPreparedConversation = (
    error: unknown,
    threadId: string,
    clientId = "",
  ) => {
  ...
  const app = express();
  const agentRoute = (routePath: string) =>
    options.listen === false ? routePath : `/agent${routePath}`;
  ...
  app.put("/canvas/projects", (_req, res) => {
    res
      ...
      .json({
        ...
        error: "已停用整批画布覆盖，请使用画布增量命令",
      });
  });
  app.get(
    "/runtime/generation-logs",
    route(async (req, res) => {
      const status = [
        "queued",
        "running",
        ...
        "failed",
        "cancelled",
      ].includes(String(req.query.status))
        ? (String(req.query.status) as any)
        : undefined;
      const options = {
        projectId: stringQuery(req.query.projectId),
        nodeId: stringQuery(req.query.nodeId),
        status,
        limit: Number(req.query.limit || 500),
      };
      res.json({
        ok: true,
        logs: (await backend.listGenerationLogs(options)).slice(
          0,
          options.limit,
        ),
      });
    ...
    route(async (req, res) => {
      ...
      if (!body.projectId || !body.platform || !body.startedAt)
        return void res
          ...
          .json({
            ok: false,
            error: "projectId、platform、startedAt 为必填项",
          });
      res
        .status(201)
        .json({
          ok: true,
          log: await backend.createGenerationLog(body as any),
        });
    }),
  );
  app.patch(
    "/runtime/generation-logs/:id",
    route(async (req, res) => {
      const body = await generationLogPatch(req.body, backend);
      res.json({
        ok: true,
        log: await backend.updateGenerationLog(
  ...
  app.delete(
    ...
    route(async (req, res) => {
      ...
      if (
        !id &&
        !stringQuery(req.query.projectId) &&
        !stringQuery(req.query.nodeId)
      )
        return void res
          .status(400)
      ...
      res.json({
        ok: true,
        deleted: await backend.deleteGenerationLogs({
          id,
          projectId: stringQuery(req.query.projectId),
          nodeId: stringQuery(req.query.nodeId),
        }),
  ...
  app.post(
    ...
    route(async (req, res) => {
      ...
      const storageKey =
        typeof rawStorageKey === "string" && rawStorageKey.trim()
          ? rawStorageKey.trim()
          : "";
      ...
      if (!storageKey && !dataUrl)
        return void res
          .status(400)
          .json({ ok: false, error: "缺少 dataUrl 或 storageKey" });
      res
        .status(201)
        .json({
          ok: true,
          media: await backend.runtimeMediaStore(
            name,
            dataUrl,
  ...
  app.get(
    ...
    route(async (req, res) => {
      ...
      try {
        ...
        res.status(404).json({ ok: false, error: "媒体文件不存在" });
      }
    }),
  );
  app.get(
    "/comfy/status",
    route(async (_req, res) =>
      res.json({ ok: true, ...(await comfyUi.status()) }),
    ),
  );
  app.get(
    "/comfy/models",
    route(async (_req, res) =>
      res.json({ ok: true, data: await comfyUi.models() }),
    ),
  );
  app.get(
    "/comfy/media",
    route(async (req, res) => {
      ...
      const response = await fetch(
        `${await comfyUi.getUrl()}/view?${query.toString()}`,
      );
  ...
  app.get(
    "/comfy/config",
    route(async (_req, res) =>
      res.json({ ok: true, url: await comfyUi.getUrl() }),
    ),
  );
  app.put(
    "/comfy/config",
    route(async (req, res) =>
      res.json({
        ok: true,
        url: await comfyUi.setUrl(String(req.body?.url || "")),
      }),
    ),
  );
  app.get("/comfy/presets", (_req, res) =>
    res.json({ ok: true, data: comfyUi.presets() }),
  );
  app.post(
    "/comfy/tasks",
    route(async (req, res) =>
  ...
  app.get(
    ...
    route(async (req, res) => {
      try {
        const { task, events } = await resolveComfyTask(
          backend,
          routeParam(req.params.id),
  ...
  app.post(
    "/comfy/tasks/:id/cancel",
    route(async (req, res) =>
      res.json({
        ok: true,
        task: await comfyUi.cancel(routeParam(req.params.id)),
      }),
    ),
  );
  app.get(
    agentRoute("/attachments/:attachmentId"),
    route(async (req, res) => {
  ...
  app.get(
    agentRoute("/message-assets/:messageKey/:assetFile"),
    route(async (req, res) => {
      const asset = await messageMetadataStore.readAsset(
        routeParam(req.params.messageKey),
        routeParam(req.params.assetFile),
      );
      if (!asset)
        return void res
          .status(404)
          .json({ ok: false, error: "message asset not found" });
      res.setHeader("Cache-Control", "private, max-age=31536000, immutable");
      res.type(asset.contentType).send(asset.data);
    }),
  );
  app.post(
    agentRoute("/local-file/reveal"),
    route(async (req, res) => {
      const filePath = String(req.body?.path || "");
      if (!path.isAbsolute(filePath))
        return res
          .status(400)
          .json({ ok: false, error: "文件路径必须是绝对路径" });
    ...
    route(async (req, res) => {
      ...
      if (isCollaborationTool(name))
        return void res.json({
          ok: true,
          result: await executeCollaborationTool(backend, name, input, {
      ...
      if (name === "comfyui_status")
        return void res.json({ ok: true, result: await comfyUi.status() });
      if (name === "comfyui_get_task") {
        try {
          const { task } = await resolveComfyTask(
            backend,
            String(input.taskId || ""),
          );
      ...
      if (name === "drama_list_episodes")
        return void res.json({
          ok: true,
          result: await backend.listDramaEpisodes(String(input.dramaId || "")),
        });
      if (name === "drama_get_episode")
        return void res.json({
          ok: true,
          result: await backend.getDramaEpisode(String(input.episodeId || "")),
        });
      if (name === "drama_create_episode") {
        const { dramaId, ...episode } = input;
        return void res.json({
          ok: true,
          result: await backend.createDramaEpisode(
            String(dramaId || ""),
            episode,
          ),
        });
      ...
      if (name === "drama_update_episode") {
        ...
        return void res.json({
          ok: true,
          result: await backend.updateDramaEpisode(
            String(episodeId || ""),
            patch,
          ),
        });
      }
      if (name === "drama_delete_episode")
        return void res.json({
          ok: true,
          result: await backend.deleteDramaEpisode(
  ...
  app.post(
    "/api/plugins/mcp",
    route(async (req, res) => {
      const declarations = Array.isArray(req.body?.plugins)
        ? (req.body.plugins as PluginMcpDeclaration[])
        : [];
      await savePluginMcpDeclarationsToBackend(backend, declarations);
      res.json({
        ok: true,
        plugins: declarations.map((declaration) => ({
          id: declaration.id,
          enabled: declaration.mcp?.enabled ?? false,
        })),
      });
    }),
  );
  app.get(
    ...
    route(async (_req, res) => {
      res.json({
        ok: true,
        plugins: await loadPluginMcpDeclarationsFromBackend(backend),
      });
    }),
  );
  ...
  app.post(
    agentRoute("/codex/skills/draft"),
    codexMutation(async (req, res) => {
      const workspace = ensureSiteWorkspace(config);
      const source = String(req.body?.source || "");
      if (source !== "conversation" && source !== "canvas")
        return res.status(400).json({ ok: false, error: "Skill 草稿来源无效" });
      ...
      try {
        if (source === "conversation") {
          ...
          if (!threadId)
            return res
              .status(409)
              .json({ ok: false, error: "当前没有可提炼的对话" });
          ...
          if (
            ...
            return res
              .status(409)
              .json({ ok: false, error: "当前对话还没有可提炼的已完成内容" });
        ...
        if (
          !snapshot ||
          (snapshot as Record<string, unknown>).hasCanvas === false
        )
          return res
            .status(409)
            .json({ ok: false, error: "当前页面没有可提炼的画布" });
        session.setCodexState(
  ...
  app.get(
    ...
    route(async (req, res) => {
      res.json({
        ok: true,
        data: await skillStore.get(routeParam(req.params.name)),
      });
    }),
  );
  app.post(
  ...
  app.post(
    agentRoute("/codex/skills/:name/enabled"),
    codexMutation(async (req, res) => {
      if (typeof req.body?.enabled !== "boolean")
        return res.status(400).json({ ok: false, error: "Skill 启用状态无效" });
      const workspace = ensureSiteWorkspace(config);
  ...
  app.post(
    agentRoute("/codex/skills/:name/delete"),
    codexMutation(async (req, res) => {
      await skillStore.delete(
        routeParam(req.params.name),
  ...
  app.post(
    agentRoute("/codex/skills/:name"),
    codexMutation(async (req, res) => {
      const data = await skillStore.update(
        routeParam(req.params.name),
        req.body,
      );
      session.emitAll("skills_changed", { forceReload: true });
  ...
  app.post(
    ...
    codexMutation(async (req, res) => {
      ...
      res.json({
        ok: true,
        workspace: ensureSiteWorkspace(config),
        conversation: session.conversationStateSnapshot,
  ...
  app.post(
    agentRoute("/codex/threads/reset"),
    codexMutation(async (req, res) => {
      const clientId = String(req.body?.clientId || "");
      session.beginConversation({ sourceClientId: clientId });
      setActiveThread(
  ...
  app.get(
    agentRoute("/codex/threads/:threadId"),
    route(async (req, res) => {
      const workspace = ensureSiteWorkspace(config);
  ...
  app.post(agentRoute("/codex/history/ack"), (req, res) => {
    const threadId = String(req.body?.threadId || "");
    const turnIds = Array.isArray(req.body?.turnIds)
      ? req.body.turnIds.map(String)
      : [];
    session.acknowledgeCodexHistory(threadId, turnIds);
    res.json({ ok: true });
  });
  app.post(
    agentRoute("/codex/threads/:threadId/resume"),
    codexMutation(async (req, res) => {
      const threadId = routeParam(req.params.threadId);
      const clientId = String(req.body?.clientId || "");
      try {
        const result = await prepareExistingThread(
          threadId,
          clientId,
          permissionMode(req.body?.permissionMode),
        );
  ...
  app.post(
    agentRoute("/codex/threads/:threadId/delete"),
    codexMutation(async (req, res) => {
      const workspace = ensureSiteWorkspace(config);
      const threadId = routeParam(req.params.threadId);
      ...
      const nextWorkspace = setActiveThread(
        ...
        { sourceClientId: String(req.body?.clientId || "") },
      );
      res.json({
        ok: true,
        workspace: nextWorkspace,
  ...
  app.post(
    ...
    codexMutation(async (req, res) => {
      ...
      if (
        ...
        (expectedRevision && expectedRevision !== conversation.revision)
      ) {
        return res
          .status(409)
          .json({
      ...
      const messageMetadata = await messageMetadataStore.recordPending(
        messageId,
      ...
      try {
        ...
        session.emitThread("chat_message", threadId, {
          sourceClientId: clientId,
          message: {
            id: `${threadId}:pending:synthetic:user`,
        ...
        const lifecycleEmit = (type: string, payload: unknown) => {
          ...
          const eventThreadId = String(
            value.threadId || value.thread_id || threadId,
          );
          ...
          session.emitThread(type, eventThreadId, {
            ...
            ...(eventTurnId
              ? { turnId: eventTurnId, turn_id: eventTurnId }
              : {}),
            ...(sourceClientId ? { sourceClientId } : {}),
          });
        };
          ...
          attachments,
          {
            ...
            onThread: (actualThreadId) => {
              ...
              void messageMetadataStore
                ...
                .catch((error) =>
                  logger.warn("Failed to bind message metadata to thread", {
                    clientMessageId: messageId,
            ...
            onTurn: (actualTurnId) => {
              turnId = actualTurnId;
              void messageMetadataStore
                .bindTurn(messageId, threadId, turnId)
                .catch((error) =>
                  logger.warn("Failed to bind message metadata to turn", {
                    clientMessageId: messageId,
                    threadId,
                    turnId,
                    error,
              ...
              if (chatTurnId !== turnId) {
                ...
                session.emitThread("chat_message", threadId, {
                  ...
                  sourceClientId: clientId,
                  message: {
                    id: `${threadId}:${turnId}:synthetic:user`,
                    itemId: "synthetic:user",
                    clientMessageId: messageId,
                    threadId,
                    turnId,
            ...
            onFinish: () => {
              ...
              if (!turnId)
                void messageMetadataStore
                  ...
                  .catch((error) =>
                    logger.warn("Failed to remove unbound message metadata", {
                      clientMessageId: messageId,
                      error,
                    }),
                  );
        ...
        await messageMetadataStore
          .remove(messageId, threadId)
          .catch((metadataError) =>
            logger.warn("Failed to remove rejected message metadata", {
              clientMessageId: messageId,
              error: metadataError,
            }),
          );
  ...
  app.use((error: Error, req: Request, res: Response, _next: NextFunction) => {
    ...
    if (
      error instanceof SkillStoreError ||
      error instanceof CodexSkillLookupError
    )
      return void res
  ...
  const recoverActiveThread = () => {
    ...
    void prepareExistingThread(activeThreadId)
      .catch(async (error) => {
        if (!isRecoverableThreadError(error))
          return failPreparedConversation(error, activeThreadId);
        session.beginConversation();
        setActiveThread("", { emptyThread: true, draftThread: true }, true);
        await prepareDraftThread("", "request");
      })
      .finally(() => session.endCodexMutation())
      .catch(() => undefined);
  };
...
async function generationLogBody(
  value: unknown,
  ...
  return {
    ...
    nodeId: stringQuery(body.nodeId),
    segmentId: stringQuery(body.segmentId),
    status: ["queued", "running", "success", "failed", "cancelled"].includes(
      String(body.status),
    )
      ? String(body.status)
      : "queued",
    platform: String(body.platform || "Generate"),
    workflow: stringQuery(body.workflow),
    model: stringQuery(body.model),
    taskMode: stringQuery(body.taskMode),
    prompt: typeof body.prompt === "string" ? body.prompt : "",
    references: await sanitizeLogMedia(body.references, backend),
    inputCounts: objectNumberMap(body.inputCounts),
    runtimeTaskId: stringQuery(body.runtimeTaskId),
    promptId: stringQuery(body.promptId),
    startedAt: String(body.startedAt || new Date().toISOString()),
    finishedAt: stringQuery(body.finishedAt),
    durationMs: Math.max(0, Number(body.durationMs || 0)),
    outputs: await sanitizeLogMedia(body.outputs, backend),
    error: stringQuery(body.error),
    params: objectRecord(body.params),
  };
}

async function generationLogPatch(
  value: unknown,
  backend: ReturnType<typeof createBackendClient>,
) {
  const body = await generationLogBody(
    {
  ...
  for (const key of [
    ...
    if (key in source) result[key] = (body as any)[key];
...
function objectNumberMap(value: unknown): Record<string, number> {
  return Object.fromEntries(
    Object.entries(objectRecord(value)).map(([key, item]) => [
      key,
      Number(item) || 0,
    ]),
  );
...
async function sanitizeLogMedia(
  value: unknown,
  backend: ReturnType<typeof createBackendClient>,
  ...
  return (
    await Promise.all(
      value.map(async (item) => {
        ...
        for (const key of ["dataUrl", "base64"])
          if (
            typeof copy[key] === "string" &&
            String(copy[key]).startsWith("data:")
          )
            delete copy[key];
        ...
        if (url.startsWith("data:") && !copy.storageKey) {
          try {
            const uploaded = await backend.uploadMedia({
              name,
              dataUrl: url,
            ...
            logger.warn(
              "[generation-logs] failed to persist dataURL reference",
              {
                name,
...
function startupStatus(
  ...
  return value === "starting" || value === "ready" || value === "failed"
    ? value
    : "cancelled";
}
...
function mcpInventory(value: unknown) {
  ...
  return value.flatMap((item) => {
    ...
    return name
      ? [{ name, authStatus: String(server.authStatus || "") || undefined }]
      : [];
  });
}
...
function skillSelector(value: unknown): CodexSkillSelector {
  ...
  if (!name || !skillPath)
    throw new CodexSkillLookupError("Skill 选择无效", 400);
