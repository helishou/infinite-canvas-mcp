...
export function registerAgentRuntimeRoutes(app: Express, stores: Stores, runningHub: RunningHubBackend, videoConcat: VideoConcatBackend) {
    ...
    app.post("/agent/runninghub/tasks", async (req, res) => res.status(202).json({ ok: true, task: await runningHub.run(objectBody(req.body?.input), objectBody(req.body?.params)) }));
    app.get("/agent/runninghub/tasks/:id", (req, res) => taskResponse(req.params.id, "runninghub:", stores, req, res));
    app.post("/agent/runninghub/tasks/:id/cancel", (req, res) => res.json({ ok: true, task: runningHub.cancel(req.params.id) }));
    app.get("/agent/ffmpeg/status", async (_req, res) => res.json({ ok: true, ...(await videoConcat.status()) }));
    app.post("/agent/video-concat/tasks", async (req, res) => res.status(202).json({ ok: true, task: await videoConcat.run(Array.isArray(req.body?.videos) ? req.body.videos.map(String) : [], String(req.body?.output || ""), req.body?.longEdge === "auto" || req.body?.longEdge === undefined ? "auto" : Number(req.body.longEdge)) }));
