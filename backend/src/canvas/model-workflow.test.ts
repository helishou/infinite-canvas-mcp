            ...
            models: [
                {
                    name: "四视图",
                    capability: "image",
                    workflows: ["custom/图生四视图.json", "custom/krea2人物多角度图.json"],
                    workflowRouting: { text: "custom/krea2人物多角度图.json", single: "custom/图生四视图.json", multi: WORKFLOW_ROUTE_UNSUPPORTED },
                },
                {
                    name: "krea2",
                    capability: "image",
                    workflows: ["custom/krea2改图.json", "custom/krea2双图编辑.json"],
                    workflowRouting: { text: WORKFLOW_ROUTE_UNSUPPORTED, single: "custom/krea2改图.json", multi: "custom/krea2双图编辑.json" },
...
test("decodeChannelModel / modelOptionName 拆渠道前缀", () => {
    assert.deepEqual(decodeChannelModel(`${CHANNEL_ID}::krea2`), { channelId: CHANNEL_ID, model: "krea2" });
...
test("findChannelModel：既能按 channelId::model 也能按纯模型名定位", () => {
    assert.equal(findChannelModel(config, `${CHANNEL_ID}::krea2`)?.model.name, "krea2");
...
test("只配 workflows 没配 routing → 用列表第一个", () => {
    const cfg = { channels: [{ id: "c", models: [{ name: "m", workflows: ["custom/a.json", "custom/b.json"] }] }] };
    const r = resolveWorkflowForModel(cfg, "c::m", 2);
    assert.equal(r.ok && r.workflow, "custom/a.json");
});

test("显式 workflow 参数优先级最高", () => {
    const r = resolveWorkflowForModel(config, "krea2", 2, "custom/另一个.json");
    assert.equal(r.ok && r.workflow, "custom/另一个.json");
});

test("工作流路径作为模型名 → 直通（保持既有 MCP 行为不回归）", () => {
    const r = resolveWorkflowForModel(config, "custom/krea2改图.json", 1);
    assert.equal(r.ok && r.workflow, "custom/krea2改图.json");
    assert.equal(builtinWorkflowName("custom/krea2改图.json"), "custom/krea2改图.json");
    assert.equal(builtinWorkflowName("Z-Image.json"), "Z-Image.json");
});

test("ComfyUI 渠道未挂路由时以模型名作为工作流，前端和 MCP 无需各自推断", () => {
    const r = resolveWorkflowForModel(config, `${CHANNEL_ID}::custom/seevr图像放大.json`, 1);
    assert.equal(r.ok && r.workflow, "custom/seevr图像放大.json");
});
...
test("未配置的模型名 → no-workflow（不回退到别的模型）", () => {
    ...
    assert.match(workflowResolutionMessage(r as never), /没有可用工作流/);
});
...
test("usesWorkflowExecutor：ComfyUI 渠道统一走工作流，API 渠道保持直连", () => {
    ...
    assert.equal(usesWorkflowExecutor(config, `${CHANNEL_ID}::custom/seevr图像放大.json`), true);
