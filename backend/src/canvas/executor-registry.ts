...
const modelRegistry: CanvasModelDeclaration[] = [
    { id: "gpt-image-*", modes: ["image"], inputRoles: ["prompt", "image"], executor: "direct-image", matches: (model) => /^gpt-image(?:-|$)/i.test(model) },
...
export function listCanvasModels(): CanvasModelDeclaration[] {
    return modelRegistry.map(({ matches: _matches, ...declaration }) => ({ ...declaration }));
}
