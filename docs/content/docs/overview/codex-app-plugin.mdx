...
```text
Install the Infinite Canvas Codex plugin from https://github.com/helishou/infinite-canvas-mcp.git.
...
```bash
mkdir -p ~/plugins
git clone https://github.com/helishou/infinite-canvas-mcp.git ~/plugins/infinite-canvas
...
Reinstall or refresh the plugin after changing its manifest, MCP configuration, Skills, exposed MCP tools, app, or entry scripts. The plugin connects directly to the resident Backend Streamable HTTP MCP endpoint:
