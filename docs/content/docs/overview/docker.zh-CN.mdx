...
主应用镜像只构建静态前端并由 nginx 提供，不包含本地 Backend、Canvas Agent、MCP 与 H3 短片生产栈。该方式只适合界面预览；完整运行需要在本机启动 Backend 并持久化其 SQLite 数据库与媒体目录，可用[快速开始](/zh-CN/docs/overview/quick-start)里的三进程本地模式。
