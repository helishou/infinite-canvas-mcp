...
Codex 直接连接 Backend 的 Streamable HTTP MCP；Canvas Agent 只负责浏览器连接与对话，不作为 MCP 工具服务端。
...
将 Claude Code 配置为 HTTP MCP，地址为 `http://127.0.0.1:17370/mcp`，与 Codex 共用 Backend 工具服务。
