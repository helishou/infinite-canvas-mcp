...
Snapshots contain the canvas ID and title, nodes, edges, and selected node IDs. The browser viewport is local UI state: it is neither included in MCP snapshots nor changed by MCP. Canvas Agent keeps the latest snapshot in memory, so tools read structured state rather than the DOM.

## Applying operations

For a node creation, Codex calls an MCP tool, Canvas Agent converts it to `canvas_apply_ops`, sends an SSE `tool_call`, and the browser applies content operations such as `add_node`, `update_node`, `delete_node`, `connect_nodes`, and `select_nodes`. The browser posts the result to `/canvas/result`, which completes the MCP call. Viewport center and zoom remain browser-local.
