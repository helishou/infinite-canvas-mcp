...
export async function syncReferenceCatalog(segments: H3Segment[], synced: Map<string, string>, upsert: CanvasReferenceService["upsert"], upsertMany?: CanvasReferenceService["upsertMany"]) {
    const pending = segments.flatMap((segment) => (segment.referenceBindings || []).flatMap((binding) => {
        ...
        const asset = { id: binding.assetId, label: binding.label, mediaType: inferH3ReferenceMediaType(binding), role: binding.role, tags: binding.tags || [], url: binding.url, storageKey: binding.storageKey, mimeType: binding.mimeType, sourceNodeId: binding.sourceNodeId, subjectId: binding.subjectId };
