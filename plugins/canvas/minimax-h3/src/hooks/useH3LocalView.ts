...
import { readDefaultLayoutPanes } from "../services/h3-defaults";
...
export function useH3LocalView(ctx: CanvasNodeContext): CanvasNodeContext {
    ...
    // ③ 默认布局快照：这个节点还没有任何自己的布局值（老节点、或拖动后被刷新丢过）时，
    //    用「设为默认参数」里保存的模块宽高，而不是下面那套内置的 220/960/480/320/150。
    // 少任一层，默认布局一渲染就被硬编码值顶掉，表现成「布局没保存成功 / 刷新后还原」。
    const metadata = (shared: Record<string, unknown>, state: Record<string, unknown>) => {
        ...
        const defaults = readDefaultLayoutPanes();
        return {
            ...shared,
            ...Object.fromEntries(Object.keys(H3_LOCAL_VIEW_DEFAULTS).map((key) => [key, state[key] ?? shared[key] ?? segment?.[key] ?? defaults[key] ?? H3_LOCAL_VIEW_DEFAULTS[key as keyof typeof H3_LOCAL_VIEW_DEFAULTS]])),
        };
    };
    return {
        ...ctx,
