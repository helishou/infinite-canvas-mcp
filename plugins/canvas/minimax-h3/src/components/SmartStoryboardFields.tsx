...
export function SmartStoryboardFields({ ctx, metadata, uploads, onUpload, onPickCanvas, onRemove, onReorder }: Props) {
    ...
    return <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <label style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}><span>段间接续</span><Switch checked={metadata.smartStoryboardContinuityEnabled !== false} onChange={(checked) => ctx.updateMetadata({ smartStoryboardContinuityEnabled: checked })} checkedChildren="接续" unCheckedChildren="独立" /></label>
        ...
        <div><div style={{ marginBottom: 5, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "space-between" }}><span>参考图片</span><button type="button" onClick={() => onPickCanvas(Math.max(0, uploads.findIndex((item) => !item)))} style={{ padding: "6px 16px" }}>从画布选择</button></div><div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4 }}>{Array.from({ length: Math.max(8, uploads.length) }, (_, index) => { const ref = uploads[index]; return <label key={index} draggable={Boolean(ref)} onDragStart={(event) => { if (!ref) return; event.stopPropagation(); event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("application/x-smart-storyboard-slot", String(index)); }} onDragOver={(event) => { event.preventDefault(); event.stopPropagation(); }} onDrop={(event) => handleDrop(event, index)} style={{ position: "relative", flex: "0 0 172px", height: 172, border: `1px solid ${ctx.theme.node.stroke}`, borderRadius: 6, overflow: "hidden", cursor: ref ? "grab" : "pointer", background: ctx.theme.node.panel, display: "flex", alignItems: "center", justifyContent: "center" }}>{ref ? <><img src={ref.url} alt={`图片${index + 1}`} draggable={false} style={{ width: "100%", height: "100%", objectFit: "cover" }} /><span style={{ position: "absolute", left: 4, bottom: 4, padding: "1px 4px", background: "rgba(0,0,0,.72)", color: "#fff", fontSize: 22 }}>图片{index + 1}</span><button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); onRemove(index); }} style={{ position: "absolute", right: 3, top: 3, width: 20, height: 20, padding: 0 }}>×</button></> : <><span style={{ fontSize: 25, color: ctx.theme.node.muted }}>＋</span><span style={{ position: "absolute", bottom: 4, fontSize: 22, color: ctx.theme.node.muted }}>图片{index + 1}</span></>}<input type="file" accept="image/*" onChange={(event) => { const file = event.target.files?.[0]; if (file) onUpload(file, index); event.currentTarget.value = ""; }} style={{ display: "none" }} /></label>; })}</div><div style={{ marginTop: 4, color: ctx.theme.node.muted, fontSize: 12 }}>可点击“从画布选择”加入图片，也可点击空槽上传本地文件；拖动已上传图片可调整槽位顺序。</div></div>
