...
type H3TimelineProps = {
    ...
    total: number;
    refLanes: number;
    onRemoveRef: (ref: H3Ref) => void;
    fmt: (value: number) => string;
};

export function H3Timeline({ ctx, segments, selected, total, refLanes, onRemoveRef, fmt }: H3TimelineProps) {
    const addRef = (event: React.DragEvent<HTMLDivElement>) => {
        ...
        if (targetRefs.some((item) => item.url === ref.url)) return;
        const next = [...targetRefs, ref];
        ctx.updateMetadata({ selectedSegmentId: target.id, segments: segments.map((item) => item.id === target.id ? withSegmentRefs(item, next) : item) });
    };
    const addSegment = () => {
