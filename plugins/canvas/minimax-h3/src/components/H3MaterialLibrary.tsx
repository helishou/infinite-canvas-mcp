...
export function H3MaterialLibrary({ ctx, assets, outputs, segments, selected, selectedRefs, imageRefs, videoRefs, audioRefs, patchSelected }: Props) {
    ...
    const section = (title: string, icon: "database" | "output", items: H3Ref[], output = false) => <>
        <div className={`minimax-library-head${output ? " minimax-output-head" : ""}`}><H3Icon name={icon} /> <span>{title}</span>{output ? <span className="minimax-output-actions"><button type="button" aria-label={outputFilter === "all" ? "切换为只显示当前 Clip 输出" : "切换为显示全部输出"} aria-pressed={outputFilter === "current"} title={outputFilter === "all" ? "当前显示全部输出，点击只显示当前 Clip" : "当前只显示当前 Clip，点击显示全部输出"} onClick={() => setOutputFilter(outputFilter === "all" ? "current" : "all")} className={`minimax-output-filter${outputFilter === "current" ? " active" : ""}`}><H3Icon name={outputFilter === "all" ? "filter-all" : "filter-current"} /></button><button type="button" aria-label="清理未用于 Clip 的输出" title="清理未用于 Clip 的输出" onClick={clearUnused} className="minimax-output-clear"><H3Icon name="trash" /></button></span> : null}</div>
