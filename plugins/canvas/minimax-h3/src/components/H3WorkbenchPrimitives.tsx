...
export function H3PreviewPlayer({ ctx, url, kind, playhead, timelineOffset = 0, playRequest }: { ctx: CanvasNodeContext; url: string; kind: H3Ref["type"]; playhead: number; timelineOffset?: number; playRequest: number }) {
    ...
    if (kind === "audio") return <div className="minimax-player-content"><div className="minimax-player-empty"><audio ref={(node) => { mediaRef.current = node; }} src={url} controls preload="metadata" /></div></div>;
    return <div className="minimax-player-content"><video ref={(node) => { mediaRef.current = node; }} src={resultUrl(url)} controls muted playsInline onLoadedMetadata={(event) => { event.currentTarget.currentTime = Math.max(0, playhead); }} onTimeUpdate={(event) => { const time = timelineOffset + Number(event.currentTarget.currentTime || 0); if (Math.abs(time - Number(ctx.node.metadata?.playhead || 0)) > 0.2) ctx.updateMetadata({ playhead: time }); }} /></div>;
}
