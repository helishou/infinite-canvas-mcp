...
export function H3WorkbenchToolbar({ ctx, metadata, segments, selected, selectedIndex, playhead, total, fmt, onOpenStoryboard }: Props) {
    ...
    return <>
        ...
        <div className="minimax-wb-toolbar">
            ...
            <div className="minimax-top-actions"><button type="button" title="下载当前片段" disabled={!selected?.result} onClick={() => { if (selected?.result) saveAs(resultUrl(selected.result), `Clip-${selectedIndex + 1}.mp4`); }}><H3Icon name="download" /></button><button type="button" title="打开参数" onClick={() => ctx.openPanel()}><H3Icon name="settings" /></button></div>
        </div>
    </>;
}
