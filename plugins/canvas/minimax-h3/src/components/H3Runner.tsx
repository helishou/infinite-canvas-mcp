...
export function H3Runner({ ctx }: { ctx: CanvasNodeContext }) {
    ...
    const run = async (runFromCurrent = false) => {
        ...
        update({ prompt: livePrompt, duration: liveDuration, aspectRatio: liveRatio, megapixels: liveMegapixels, videoSteps: liveSteps, denoise: liveDenoise, seed: String(liveSeed).trim() ? Number(liveSeed) : undefined, modelName: liveModelName, minimaxBaseModel: liveModelName, loraName: liveLoraName, combatLoraWeight: Number(liveSelected?.combatLoraWeight ?? liveMetadata.minimaxCombatLoraWeight ?? combatLoraWeight), cinematicLoraWeight: Number(liveSelected?.cinematicLoraWeight ?? liveMetadata.minimaxCinematicLoraWeight ?? cinematicLoraWeight), teAccel: liveMetadata.minimaxGlobalTeAccel === true || teAccel, motionContextEnabled: liveMotion, motionContextNoiseEnabled: liveSelected?.motionContextNoiseEnabled === true || liveMetadata.motionContextNoiseEnabled === true || motionNoise, motionContextNoiseAlpha: Number(liveSelected?.motionContextNoiseAlpha ?? liveMetadata.motionContextNoiseAlpha ?? noiseAlpha), motionContextNoiseAlphaEnd: Number(liveSelected?.motionContextNoiseAlphaEnd ?? liveMetadata.motionContextNoiseAlpha ?? noiseAlphaEnd), motionContextNoiseRampFrames: Number(liveSelected?.motionContextNoiseRampFrames ?? liveMetadata.motionContextNoiseRampFrames ?? noiseRampFrames), model: selectedModel, status: "loading", errorDetails: "", runStartedAt: Date.now(), cancelRequested: false, runtimeTaskId: "", runtimeRunId, runtimeTargetSegmentId: liveSelectedId, segments: liveSegments.map((segment) => relevantSegments.some((item) => item.id === segment.id) ? { ...segment, runtimeTaskId: "", progress: 0 } : segment) });
        try {
            // 日志写入也属于运行链路的一部分；后台断开时必须进入统一 catch，
            // 不能让异常浮出后把节点永远留在“生成中”。
            const generationLog = await createH3Log(ctx, liveSelected, livePrompt, logRefs, { engine: liveMetadata.minimaxEngine || "comfyui", workflow: "MiniMax H3", modelName: liveModelName, taskMode: liveSelected?.taskMode || "r2v", duration: liveDuration, aspectRatio: liveRatio, megapixels: liveMegapixels, videoSteps: liveSteps, denoise: liveDenoise, seed: liveSeed, runtimeRunId });
            ...
            for (const [index, segment] of storedSegments.entries()) {
                ...
                const h3Runner = (promptText: string, inputData: any, paramsData: any, optionsData: LocalH3Options) => {
                    if (generationLogId) void ctx.generationLogs.update(generationLogId, {
                        params: { runtimeRunId, runFromCurrent, selectedSegmentId: liveSelectedId, segments: [...submittedSegments], lastSubmitted: { prompt: promptText, input: inputData, params: paramsData } },
                ...
                if (generationLogId) {
                    await ctx.generationLogs.update(generationLogId, {
                        ...
                        params: { runtimeRunId, runFromCurrent, selectedSegmentId: liveSelectedId, segments: [...submittedSegments], lastSubmitted: { prompt: submittedPrompt, input: submittedInput, params: submittedParams } },
            ...
            if (generationLogId) void ctx.generationLogs.update(generationLogId, { status: cancelled ? "cancelled" : "failed", finishedAt: new Date().toISOString(), durationMs: Date.now() - Number(liveMetadata.runStartedAt || Date.now()), error: enhancedError, params: { ...lastSubmitted } }).catch((logError) => console.warn("[minimax-h3] failed to record failed generation", logError));
        } finally {
