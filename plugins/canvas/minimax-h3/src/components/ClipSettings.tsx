...
export function ClipSettings({ ctx, metadata, segment, patch }: Props) {
    const [catalog, setCatalog] = useState<{ models: string[]; loras: string[]; textEncoders: string[]; videoVaes: string[]; audioVaes: string[]; nanfeng: Record<string, unknown[]> }>({ models: [], loras: [], textEncoders: [], videoVaes: [], audioVaes: [], nanfeng: {} });
    useEffect(() => { let active = true; void discoverH3Models(ctx).then((value) => { if (active) setCatalog(value); }); return () => { active = false; }; }, [ctx]);
    ...
    const section = (key: SectionKey, title: string, summary: string, content: ReactNode, className = "") => <section className={`nfh3-section ${className}`}><button type="button" className="nfh3-section-head" onClick={() => setOpen(key)}><span className="nfh3-chevron">{expanded[key] === true ? "⌄" : "›"}</span><b>{title}</b><small>{summary}</small></button>{expanded[key] === true ? <div className="nfh3-section-body">{content}</div> : null}</section>;
    ...
    return <div className="nfh3-settings">
        ...
        {section("sampling", "采样设置", `${segment.steps || 20} 步 · ${segment.sampler || "res_multistep"}`, <div className="nfh3-control-grid">
            ...
            {control("随机种子", <InputNumber style={field} min={0} value={segment.seed === undefined ? undefined : Number(segment.seed)} placeholder="留空随机" onChange={(value) => patch({ seed: value ?? undefined, noiseSeed: value ?? undefined, noiseSeedMode: value === null ? "random" : "fixed" })} />)}
        </div>)}
