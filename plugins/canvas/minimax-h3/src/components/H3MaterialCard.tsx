...
export function H3MaterialCard({ ctx, ref, compact = false, removable = false, onRestore, onRemove, onOpenPreview }: { ctx: CanvasNodeContext; ref: H3Ref; compact?: boolean; removable?: boolean; onRestore?: () => void; onRemove?: () => void; onOpenPreview?: () => void }) {
    ...
    return <div key={ref.url} className={cardClass} draggable onDragStart={dragStart} onDoubleClick={(event) => { event.stopPropagation(); onOpenPreview?.(); }} title={onOpenPreview ? "双击放大预览" : undefined} style={{ position: "relative", flex: `0 0 ${compact ? 82 : 118}px`, height: compact ? (isOutput ? 78 : 58) : 64, overflow: "hidden", border: `1px solid ${ctx.theme.node.stroke}`, borderRadius: 5, background: ctx.theme.node.fill, cursor: "grab" }}>
        {ref.type === "video" ? compactMedia ? <H3Icon name="clapperboard" /> : <video src={ref.url} muted playsInline preload="metadata" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : ref.type === "image" ? <img src={ref.url} alt={ref.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ padding: 16, fontSize: 22 }}>♫ {ref.name}</span>}
