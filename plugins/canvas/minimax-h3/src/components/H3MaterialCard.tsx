...
export function H3MaterialCard({ ctx, ref, compact = false, removable = false, onRestore, onRemove, onOpenPreview }: { ctx: CanvasNodeContext; ref: H3Ref; compact?: boolean; removable?: boolean; onRestore?: () => void; onRemove?: () => void; onOpenPreview?: () => void }) {
    ...
    return <div key={ref.url} className={cardClass} draggable onDragStart={dragStart} onDoubleClick={(event) => { event.stopPropagation(); onOpenPreview?.(); }} title={onOpenPreview ? "双击放大预览" : undefined} style={{ position: "relative", flex: `0 0 ${compact ? 82 : 118}px`, height: compact ? (isOutput ? 78 : 58) : 64, overflow: "hidden", border: `1px solid ${ctx.theme.node.stroke}`, borderRadius: 5, background: ctx.theme.node.fill, cursor: "grab" }}>
        {ref.type === "video" ? compactMedia ? <H3Icon name="clapperboard" /> : <video src={mediaUrl} muted playsInline preload="metadata" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : ref.type === "image" ? <img src={mediaUrl} alt={ref.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ padding: 16, fontSize: 22 }}>♫ {ref.name}</span>}
        <span>{ref.type === "image" ? "Image" : ref.type === "video" ? "Video" : "Audio"}</span>
        {isOutput ? <span style={{ position: "absolute", right: 4, top: 4, display: "flex", gap: 3 }}><button type="button" title="放大预览" aria-label="放大预览" onClick={(event) => { event.stopPropagation(); onOpenPreview?.(); }}><H3Icon name="zoom" /></button><button type="button" title="下载输出视频" aria-label="下载输出视频" onClick={(event) => { event.stopPropagation(); saveAs(mediaUrl, ref.name || "h3-output"); }}><H3Icon name="download" /></button>{onRestore ? <button type="button" title="设为当前 Clip（还原提示词与参数）" aria-label="设为当前 Clip（还原提示词与参数）" onClick={(event) => { event.stopPropagation(); onRestore(); }}><H3Icon name="restore" /></button> : null}</span> : null}
