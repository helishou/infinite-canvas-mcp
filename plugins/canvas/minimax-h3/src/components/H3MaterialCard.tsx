...
export function H3MaterialCard({ ctx, ref, compact = false, removable = false, onRestore, onRemove }: { ctx: CanvasNodeContext; ref: H3Ref; compact?: boolean; removable?: boolean; onRestore?: () => void; onRemove?: () => void }) {
    const isOutput = compact && String(ref.name || "").startsWith("Clip");
    ...
    return <div key={ref.url} className={cardClass} draggable={!isOutput} onDragStart={dragStart} style={{ position: "relative", flex: `0 0 ${compact ? 82 : 118}px`, height: compact ? (isOutput ? 78 : 58) : 64, overflow: "hidden", border: `1px solid ${ctx.theme.node.stroke}`, borderRadius: 5, background: ctx.theme.node.fill, cursor: isOutput ? "default" : "grab" }}>
        ...
        {isOutput ? <span style={{ position: "absolute", right: 2, top: 2, display: "flex", gap: 2 }}><button type="button" title="下载" onClick={(event) => { event.stopPropagation(); saveAs(ref.url, ref.name || "h3-output"); }}><H3Icon name="download" /></button>{onRestore ? <button type="button" title="设为当前 Clip（还原提示词与参数）" onClick={(event) => { event.stopPropagation(); onRestore(); }}><H3Icon name="output" /></button> : null}</span> : null}
