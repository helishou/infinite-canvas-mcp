...
type Props = { ctx: CanvasNodeContext; metadata: Record<string, unknown>; selected?: H3Segment; imageRefs: H3Ref[]; videoRefs: H3Ref[]; audioRefs: H3Ref[]; patchSelected: (patch: Partial<H3Segment>) => void; onOpenStoryboard: () => void };

export function H3ClipSettingsPanel({ ctx, metadata, selected, imageRefs, videoRefs, audioRefs, patchSelected, onOpenStoryboard }: Props) {
    ...
    return <div className="minimax-clip-parameters">
        ...
        <H3PromptSection ctx={ctx} selected={selected} imageRefs={imageRefs} videoRefs={videoRefs} audioRefs={audioRefs} patchSelected={patchSelected} onOpenStoryboard={onOpenStoryboard} />
