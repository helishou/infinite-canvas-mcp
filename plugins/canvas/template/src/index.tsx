...
import { definePlugin, useSyncExternalStore } from "@infinite-canvas/plugin-sdk";
...
function TemplateContent({ ctx }: CanvasNodeContentProps) {
    ...
    const spawnBelow = async () => {
        await ctx.flush();
        const latestContent = String(ctx.getNode(ctx.node.id)?.metadata?.content || content);
        const id = `template-${ctx.node.id}-${ctx.getNodes().length}`;
        ctx.applyOps([
            { type: "add_node", id, nodeType: "text", title: "衍生节点", x: ctx.node.position.x, y: ctx.node.position.y + ctx.node.height + 40, metadata: { content: latestContent, status: "success" } },
    ...
    return (
        ...
        <div data-canvas-no-zoom onMouseDown={(e) => e.stopPropagation()} style={{ height: "100%", width: "100%", display: "flex", flexDirection: "column", gap: 8, padding: 12, boxSizing: "border-box", color: ctx.theme.node.text }}>
            <div style={{ display: "flex", gap: 6 }}>
                <button type="button" style={btn} onClick={() => setEditing(!editing)}>
                    {editing ? "完成" : "编辑"}
                </button>
                <button type="button" style={btn} onClick={() => void spawnBelow()}>
                    衍生文本
                </button>
            </div>
            {editing ? (
                <ctx.TextEditor projectId={ctx.projectId} target={{ nodeId: ctx.node.id, field: "content" }} autoFocus placeholder="输入内容…" onEscape={() => setEditing(false)} style={{ flex: 1, border: "none", outline: "none", background: "transparent", color: ctx.theme.node.text, fontSize: 14, lineHeight: 1.5 }} />
            ) : (
