...
import { getGroupResourceNodes, nodeResourceItems } from "@/lib/canvas/canvas-resource-references";
...
export function CanvasNodeReferenceBar({ nodeId, nodes, connectedNodes, onDisconnect, onStartSelection }: { nodeId: string; nodes: CanvasNodeData[]; connectedNodes: CanvasNodeData[]; onDisconnect?: (fromNodeId: string, toNodeId: string) => void; onStartSelection?: (nodeId: string) => void }) {
    ...
    const references = connectedNodes.flatMap((sourceNode) => (sourceNode.type === CanvasNodeType.Group ? getGroupResourceNodes(sourceNode.id, nodes) : [sourceNode]).flatMap((node) => nodeResourceItems(node).map((resource, index) => ({ node, resource, index, sourceNodeId: sourceNode.id }))));
    return (
        <div className="mb-2">
            ...
            <div className="thin-scrollbar flex min-h-12 gap-2 overflow-x-auto pb-1">
                {references.map(({ node, resource, index, sourceNodeId }) => <ReferenceItem key={`${sourceNodeId}:${node.id}:${index}`} node={node} resource={resource} onRemove={() => onDisconnect?.(sourceNodeId, nodeId)} />)}
