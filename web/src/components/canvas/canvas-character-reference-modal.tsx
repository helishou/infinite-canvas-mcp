...
import { Image as ImageIcon, Maximize2, Music2, User } from "lucide-react";
...
export function CanvasCharacterReferenceModal({ node, selection, hideVoiceOption, onApply, onClose }: {
    node: CanvasNodeData;
    selection?: CanvasCharacterReferenceSelection;
    /** 引用本角色的生成节点是否不消费声线（生图 / 智能选生图 / 智能选文本）。是则隐藏声线选项。 */
    hideVoiceOption?: boolean;
    ...
    return (
        ...
        >
                            ...
                            {images.map((image, index) => {
                                ...
                                return <label key={`${characterReferenceKey(image, index)}:${index}`} className="group relative cursor-pointer overflow-hidden rounded-xl border transition" style={{ borderColor: checked ? theme.node.activeStroke : theme.toolbar.border, background: theme.node.panel, opacity: checked ? 1 : 0.52 }}>
                                    <div className="relative">
                                        ...
                                        <button type="button" className="absolute right-2 top-2 grid size-8 place-items-center rounded-lg border shadow-sm transition hover:scale-105" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.text }} aria-label={t("canvas.character.imagePreview")} title={t("canvas.character.imagePreview")} onClick={(event) => { event.preventDefault(); event.stopPropagation(); setPreviewImage(image.url); }}>
            ...
            {previewImage ? <Image src={previewImage} alt={t("canvas.character.imagePreview")} style={{ display: "none" }} preview={{ visible: true, src: previewImage, onVisibleChange: (visible) => { if (!visible) setPreviewImage(null); } }} /> : null}
        </Modal>
    );
}
