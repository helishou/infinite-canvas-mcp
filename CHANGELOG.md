...
- [修复] H3 Output「设为当前 Clip」不还原提示词：之前 `buildRestoreParamsPatch` 用 `H3_SETTINGS_KEYS`（已排除 `prompt`）故意不还原 prompt。现改为 `RESTORABLE_PARAM_KEYS`（含 `prompt`），使点 Output 视频「设为当前 Clip」会把源段提示词一并带入当前 clip；复制/粘贴设置（export/importH3Settings）仍保持不携 prompt。配合上面的撤销栈，误覆盖可 Ctrl+Z 回退。
