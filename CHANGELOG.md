...
- [调整] 循环轮数按输入有序组自动计算，图片/视频/音频/文本结果统一写入右侧有序组；每轮参考图连接对应结果槽，旧链式输出节点按顺序转为结果槽并保留历史。

- [修复] H3 设置面板的下拉（模型、LoRA、采样器等）在画布空白处点击时不再卡住不收起：画布空白处的 `pointerdown` 会 `preventDefault()`，浏览器因此不再派发 antd 依赖的兼容 `mousedown`；改为插件自己接管开关状态并用 `pointerdown` 捕获兜底收起。

- [调整] H3 LoRA 选择框文件名字号从 28px 降到 14px 并对超长文件名做省略，窄栏位下也能看清当前选中的 LoRA。

- [修复] 打开画布视频同屏对比弹窗时不再与画布上已经在跑的视频节点叠加播放：先暂停画布里所有 `data-canvas-video` 视频，避免关掉弹窗后画布视频还在后台出声音。

- [修复] 画布媒体预览弹窗关闭、换素材或销毁时显式 pause 当前 `<audio>` / `<video>`，降低 `autoPlay` 视频被关掉后还在后台播放的概率。

- [修复] 视频同屏对比播放时移除进度条浮层的循环更新路径，播放与跳转共用稳定的原生进度控件。

- [调整] 智能图片节点失败/取消的生成槽位自动从图片历史中清出，主图与顶层媒体字段同步回退到剩余的成功槽位；批量部分失败时只清掉失败槽，成功的继续保留。智能视频/音频/文本节点本身不预占历史槽位，行为不变。

- [修复] H3 失败父任务清理所属 Clip 的残留运行状态；重置重生先核对任务终态，避免对 `failed` 任务再次取消。

- [新增] 画布视频节点菜单先从画布点选另一个视频再进入同屏对比，统一播放、暂停和拖动进度，短视频结束后停在末帧。

- [修复] H3 LoRA 列表读取 ComfyUI 发现的所有目录，并在展开设置或手动刷新时更新；支持按文件名搜索候选，或在槽位直接输入相对文件名。

- [修复] 画布循环区分有序组轮换素材与固定参考图，任务日志显示当轮组图；工作流图片槽不足时拒绝静默丢图，Flux2-Klein 旧 Inspire 单模型加载节点在执行时适配内置节点。

- [调整] 项目定位从图片创作工作台扩展为 AI 影像与短片生产工作台：README 与文档站同步 H3 短片 Clip 生产、剧目分集、画布与 H3 MCP 工具面和生产 SOP；启动方式更正为 backend/agent/web 三进程本地模式，Docker 镜像只含前端的边界在 README、快速开始与 Docker 文档中写明，示例仓库地址改为 helishou/infinite-canvas-mcp。

- [修复] H3 分镜图引用随分镜轨和逐镜绑定顺序调整，提示词中的 Picture 编号继续指向原素材，避免分镜图与 Picture 顺序错位。

- [修复] H3 提示词编辑器识别直接粘贴的 Ref2VA 主体定义，道具等非人物 Subject 不再误报“人物未找到”；失效引用统一标为“主体”。

- [优化] MCP `canvas_get_state` 默认返回精简节点目录，布局与连线按需读取，支持 revision 未变化时返回简短回执；指定画布直接读取单项目，避免重复加载全部画布。

- [优化] 画布生图设置记住最近选用的模型、质量、尺寸、张数、透明背景和 16 倍数对齐，新图片节点沿用这些默认值。

- [修复] 有序组成员拖动后的撤销与重做保留原槽位、位置和尺寸，避免增量回放把明确恢复的布局再次自动排布。

- [调整] H3 移除单张角色服装等参考素材时不再因提示词中的 `<Picture N>` 编号阻断；Backend 识别旧页面夹带的参考副本，避免误拒绝明确的移除操作。存量失效编号不再使分镜编辑初始化崩溃，打开编辑器显示具体字段错误，完成时继续校验。
- [修复] 画布重取已提交操作的回执时按历史规则重放旧 H3 参考格式，避免旧检查点中的绑定数量触发 400；浏览器对已误标拒绝但后台有回执的草稿自动用原 operationId 取回结果，新写入仍保留数据保护校验。
- [修复] H3 生成预检遵循角色服装总开关，避免已关闭的服装目录误报缺少绑定；失败提示使用前端 Clip 序号并列出具体无效引用。
- [修复] H3 分镜编辑允许道具图片单独作为参考，不再要求每张已绑定图片都归入主体定义。
- [优化] H3 当前 Clip 标题显示可复制的内部 ID，时间轴 Clip 卡悬停可查看完整 ID，方便将前端序号与日志定位对应。

- [修复] H3 当前 Clip 的画布参考输入线读取现用绑定，分镜图等参考素材以带箭头虚线指向 H3 节点。

- [修复] 有序组向右插槽时的顺序偏移，以及缩放和新增成员后槽位、成员越框的问题；成员等比适配槽位，外部写入时同步重排。

- [修复] H3 站位 / 轴线图在结构化分镜中单列为 Picture 空间参考并生成对应保留分析，不再被当成人物身份图；浏览器与 MCP 写入规则保持一致。
- [新增] 画布 MCP 支持图片输入清单预检、按槽替换有序分镜与单独替换 H3 分镜绑定，角色节点有多张图时提交前要求明确选图。
- [新增] 画布图片工具栏接入 moyou 自动色阶工作流，处理结果由 Backend 作为新图片节点回写。
- [新增] 编辑角色时可从画布选择图片替换单张参考图，并保留该卡片已有的职责、服装说明和排序。
- [调整] 打开画布时按资产 ID 刷新角色节点的共享资料，画布修改仅在手动点击“存为角色资产”后回写资产库，各节点主图选择保持独立。
- [修复] H3 分镜轨上的分镜图「从画布替换」改为沿用原引用 ID 就地换素材：原卡片留在原位、序号与时长不变，不再留下一张空卡并把新图追加到轨尾；参考行直接删除分镜图也走同一入口，打开画布时已存在的坏卡会自动恢复成可重新绑图的空卡。
- [优化] 生成节点「参考内容」栏中的角色引用支持 hover 预览：与图片引用一致，悬停显示所选角色图的大图缩略（未选图时显示角色名）。
- [调整] MCP `canvas_get_state` 默认尽量返回全部节点摘要；超过现有输出上限时自动分页并提供 `nextNodeOffset`，也可用 `nodeLimit` 主动控制单次返回量。
- [修复] 自动生成提示词的分镜图描述不再泄漏素材名：摘要/标签只是素材名回显（如「S01-02·沈侯持婚书单人对峙·新版分镜组第2槽」）或归档编号标签（EP01 / S01-02 / group-… / *-anchor）时一律丢弃，回落到通用 storyboard frame；MCP `canvas_analyze_reference_asset` 写入与素材名同名的摘要时直接报错，避免再次污染。
- [修复] 重新编译分镜提示词时剥掉正文里残留的分镜图 cue（含 MCP 编译产物的 shot-entry keyframe 句式及其 After the keyframe 尾巴），不再把旧 cue 当正文层层叠加成重复句。
- [优化] MCP 指定节点读取只附带相关连线，画布概览的参考候选与生成目标改为精简字段，减少重复的大体积返回。
- [优化] MCP 大节点读取超限时自动回退摘要，H3 节点改为返回可核对的 Clip 索引，并将失效片段 ID 标为可恢复错误。
- [修复] H3 二采失败或重启后重新投影原父任务的待确认状态；本地全画面精修节点可对一采视频执行真实二采，并按当前 ComfyUI 模型目录解析模型与音频 VAE。
- [修复] H3 首段没有前段可承接时允许直接点击“生成当前 Clip”，保存的 Motion Context 开关仍可用于之后的连续运行。
- [修复] H3 分镜合成将实际合成图归档后提交模型和生成日志，避免内部占位地址导致生成失败。
- [修复] H3 高级设置展开时不再被布局压扁，Select 保留足够宽度显示当前值。
- [调整] H3 Clip 设置面板取消顶层浮动的「时长秒 / 画面比例 / 随机种子 / 种子值」和独立的「南风 V15 增强」分区：时长、画面比例、随机种子与种子值、TRT 视频 VAE、上一段完整视频作参考归入「模型与基础参数」，ER 求解参数与 Motion Context 续写组归入「采样设置」，TAEH3 彩色预览归入「实时预览」，DLSS 超分与补帧归入「RTX 视频超分」，各分区摘要同步反映这些开关。
- [新增] H3 预览显示真实分辨率：工作台主预览与放大灯箱（图片 / 视频）加载后在左下角 / 文件名旁标注实际像素尺寸（如 1920 × 1080），音频不显示。
- [修复] H3 静态构建统一使用宿主 React，历史输出与参考媒体按当前 Backend 的媒体 key 解析，避免画布加载崩溃及独立端口下无法预览视频。
- [修复] H3 存量 Clip 的绑定与旧参考列表不一致时，迁移保留现有绑定并将旧字段原样归档，避免总后台启动失败和参考媒体来源丢失。
- [调整] H3 改为冻结整批 Clip 输入、由原父任务处理一采决议，并统一参考绑定与历史输出的 Backend 权威写入；参考目录不再随工作台挂载反复同步。
- [优化] H3 时间轴停止滚动后才保存视图位置，任务查询改由 SQLite 索引筛选分页并覆盖超过 500 个活动任务的重启恢复。

- [修复] H3 MCP 按计划追加 Clip 时不再尝试改写 Backend 独占的节点运行状态；删除 Clip 工具已同步插件声明，客户端重载后可发现并调用，归档结果仍保留。
- [优化] H3 MCP 结构化视频计划追加支持按指定 Clip 前后原子插入，并返回写入后的片段索引，避免补段暂时落到时间线末尾或多段顺序颠倒。
- [调整] H3 一采完成后原父任务持久进入待确认态，确认二采、保留一采或放弃均沿用同一任务；多 Clip 串行暂停，任务中心与 MCP 提供明确操作入口和重启恢复。

- [优化] 画布本机草稿（未确认命令）不再整图落盘：每条命令携带的冲突判定基线改为「本次操作作用域」快照，只带它真正会读到的节点、连线与被改的项目字段，值一律原样保留。实测 231 节点画布（整图约 2.36 MB）的中位基线降到约 10 KB（**241x**）；冲突判定语义、后端提交内容与存量草稿完全不变（新旧两种基线并存读取）。
- [调整] MCP `canvas_get_state` 默认只回节点摘要（节点标识 + 状态/模型等，不再整幅 node metadata），新增可选 `nodeIds` 精确索取某几个节点的完整 metadata；需要整图导出仍用 `canvas_export_snapshot`（契约不变）。此前 231 节点画布一次调用返回约 3.3 MB，已超过 0.5 MiB 输出上限而直接失败。
- [新增] MCP 工具返回体增加 0.5 MiB 硬上限（`MCP_MAX_TOOL_OUTPUT_BYTES` 可覆盖，设为 0 关闭）：超限直接返回可自助恢复的 `OUTPUT_TOO_LARGE` 错误并给出缩小范围建议，不再把数 MB 内容交给模型；`assets_list` 同时剥离内联媒体并真正实现 `keyword`/`page`/`pageSize`（此前声明了却被忽略，每次返回全量）。
- [修复] MCP 可观测性页面的返回体均值改用后端真实的返回尺寸调用数，并统一“超大返回体”阈值为包含恰好 100,000 字符，避免分母显示 `undefined` 和边界统计不一致。
- [新增] 文本节点的右键菜单新增「复制内容」：文本类型节点与文本模式的智能生成节点（含反推提示词结果）在有正文时可通过右键一键把正文写入系统剪贴板并提示已复制；非文本节点或正文为空的节点不显示该项。

- [修复] 画布节点 Ctrl+C 无反应：①未生成/生成失败的节点（无媒体内容）此前在复制函数里静默返回，现在把节点结构（提示词、模型配置、连线）作为画布 JSON 复制，粘贴可原样重建；②富格式剪贴板写入失败（权限被拒/浏览器不支持）时降级为纯文本写入画布 JSON，再降级 execCommand，全部失败才提示；③粘贴端识别文本剪贴板里的画布 JSON（含历史格式），按节点复制还原而不会误建文字节点，跨标签页复制也可用；④焦点落在富文本编辑器（contenteditable/shortcuts-ignore 容器，如 H3 主提示词）但未选中文字时，Ctrl+C 放行为「复制节点」，有选中文本时仍走浏览器默认复制。
- [优化] 画布节点缩到概览壳/紧凑壳时补上「生成中」标记：节点底色与边框改用活动色、缩略图上叠一层活动色蒙层，右上角显示静态色点（节点在屏幕上够宽时带进度百分比），并加按屏幕像素补偿的静态光环，低倍率下也能一眼认出正在跑任务的节点；标记全部为静态样式，不引入动画，避免缩略降级反而增加持续重绘开销。
- [修复] H3 预览播放器暂停后跳回首帧：H3Workbench 传给播放器的 playhead 是已减去 Clip start 的相对值，播放器内部又减一次 timelineOffset，双重复减导致第 2 个及之后的 Clip（start>0）在暂停等触发 seek 的时刻被钳到 0 直接跳首帧。改为传绝对 playhead，与播放器「绝对时间减偏移得本地帧」的契约一致；顺带修复暂停/换段后跳首帧的关联问题：用户暂停瞬间不再触发换段（暂停竞态守卫）、换段起播失败回退常规路径、弱网断流早退 ended 对同一 src 自动重试一次、playToken/seek effect 移到槽位切换 effect 之后避免起播到不可见的旧槽。
- [新增] H3 节点的「翻译」和「增强提示词」现在进入生成日志与任务中心：插件文本调用传入归属元数据后，宿主统一登记生成日志（platform=canvas-text，进「生文」筛选，卡片显示操作标签「翻译/增强提示词」、节点/Clip、模型、耗时、输入参考图与输出文本），运行任务写入节点/Clip 归属并在任务中心展示；生成日志弹窗为生文类日志渲染可复制的「输出文本」段。日志经 runtimeTaskId 与任务互相关联，日志写入失败不影响生成。
- [调整] Backend 任务解析归属时兼容 `input.params` 里的 nodeId/segmentId（插件文本调用不能把 nodeId 放顶层，否则会触发画布回写绑定在 H3 节点上建文本结果节点）。
- [修复] Backend 浏览器文本任务在预创建结果节点后同时完成源节点和结果节点的状态回写，避免文本任务成功或失败后源节点残留运行态。
- [调整] H3 `replace_h3_segments` 改为按稳定 Clip ID 合并字段，省略的参考和运行字段继承旧值，只有显式空字段才清空。

- [新增] H3 空分镜双击进入画布「选节点作参考」模式，选中节点的图片以分镜职责直接绑定到该分镜（角色节点取主服装图）；也支持把图片素材直接拖到空分镜卡上绑定。分镜卡在等待选图时高亮、悬停显示虚线提示。
- [调整] H3 时间轴右键按区域分流：右键视频行 Clip 卡片弹出 Clip 菜单（在左边/右边添加 Clip、删除 Clip，仅剩一个 Clip 时删除置灰）；右键参考行才弹分镜菜单。视频行空白处不再拦截，交回画布默认菜单。
- [修复] `H3PreviewPlayer` 的 `isMuted` useState 从三个提前 return（空 url / 图片 / 音频）之后提升到组件顶部 hooks 区；此前预览源在「视频 ↔ 空/图片/音频」间切换会抛 "Rendered more/fewer hooks" 直接崩掉整棵节点树（选中有/无结果的 Clip 即可触发）。

- [调整] Clip 视频生产验收改为记录实际时长、允许按需要拆分或合并镜头，字幕/LOGO/水印/角标出现时允许针对当前 Clip 再抽一次，声音内容改由人工试听确认。

- [修复] H3 时间轴右键分镜菜单改为按光标下的元素（Clip 卡片 / 参考格 / 分镜轨）解析目标 Clip、按光标横向位置解析目标分镜，菜单强制固定在鼠标处并在超出视口时自动内收；修复画布缩放后按写死的 100px/秒 换算坐标导致解析到错误 Clip、菜单跑位的问题。

- [调整] H3 时间轴分镜轨取消半区「新增」按钮，改为右键弹出菜单新增分镜（支持「在某分镜后插入」「末尾追加」「移除」），空轨以「右键新建分镜」提示替代整块按钮。

- [调整] H3 时间轴新增加高约 1.5 倍的自定义横向滚动条，带可拖拽滑块与每 Clip 跳转标记，替代难点按的原生滚动条。

- [优化] H3 MCP 按项目读取并合并同项目并行请求，复用协作写入回执；Clip 可选一次读取详情，H3 任务等待改用 Backend SSE 并记录脱敏读取、编译、写入和等待耗时。

- [调整] H3 MCP 与插件声明升级到 1.5.4，`h3_get_clip` 增加可选详情字段。

- [优化] Clip 视频 Skill 将 H3 提示词增强器及参考文件纳入项目目录，并整合素材编号、模式对齐、镜头时间线、对白和声景规则。

- [修复] MiniMax H3 插件包、MCP 模块与宿主声明统一到 1.5.3，避免重复版本不一致告警。

- [修复] H3 视频/音频参考按带类型的 storageKey 自动识别，避免因缺失 URL、MIME 或误标 mediaType 被编辑器当成图片。

- [调整] H3 参考素材职责弹窗：视频参考直接显示首帧缩略图，标签字段替换为可进入提示词的描述字段。

- [修复] Canvas Agent 恢复分页存储的 Codex 对话时改用 turns/items 分页读取，避免 `thread/read(includeTurns=true)` 拒绝导致对话初始化失败。

- [调整] Codex 插件与侧边栏统一连接 Backend Streamable HTTP MCP，移除 Canvas Agent 的第二套 MCP server 注册入口。

- [新增] H3 MCP 增加按稳定 Clip ID 前移、后移或移至末尾的移动工具，并通过 Backend 协作操作持久化顺序。

- [调整] H3 MCP 支持按稳定角色组 ID 移除单段角色组及其参考绑定。

- [调整] MCP 生成任务状态查询默认最多返回 5 条记录，减少无必要的历史回显。

- [优化] 文字分镜表逐镜判断关键帧必要性，默认一镜一段 H3 视频；仅明显动作衔接或对白连续时才合并片内切镜。

- [调整] 对话镜头采用单人中近景、近景或特写正反打并保持明确视线与 180° 轴线；面部角度由表演和构图决定，双人同框仅用于必须同时可见的动作。

- [调整] 分镜关键帧默认冻结镜头动作的起始状态，承接上一镜后留出 Clip 的动作过程。

- [修复] 有序组整理和拖放优先保留明确位置及槽位顺序，避免图片闪现后被移走或落到末格。

- [调整] 镜头设计遇到有明确听者的大段台词时，按说话者与听者的单人正反打拆镜，并标清画外台词与情绪承接。

- [新增] 短片生产 SOP 为每个画布项目固定制作目录，保存剧本、文字分镜表、资产索引和进度记录；分镜图按画布有序组管理并记录组 ID，支持中断后继续。

- [修复] MCP 将节点写入有序组时同步追加 `groupSlots` 并立即按槽位落位，刷新或协作回放后仍保持组内成员关系。

- [调整] H3 Clip 整体重排按稳定 Clip ID 合并字段，省略参考字段时继承原 Clip 参考，显式空数组才清空。

- [调整] 下线 `h3_prepare_clip` MCP 对外工具，避免 Agent 将参数继承、参考绑定和 Clip 更新重复合并为循环准备步骤。

- [优化] 短片生产 SOP 改为主 Skill 路由六个生产子 Skill，各阶段内嵌所需 Neo Image 规则并自行验收产物；移除独立验收与交付 Skill，常规执行不再通读大规范或让细则回流主入口。

- [优化] H3 MCP 写入与任务状态工具改为返回紧凑摘要，片段读取拆分为总览、提示词、参考和运行参数工具，并升级插件协议版本以确保客户端刷新处理器，避免旧会话继续回传完整 H3 数据。

- [修复] 画布选择本地 IndexTTS 2.5 配音时改为通过 ComfyUI 执行并传递已连接的参考音频，不再错误要求配置 API Key。

- [修复] Agent 侧栏未打开时仍在后台建立网页连接，避免 MCP 导航持续误报“当前没有已连接网页”。

- [新增] H3 角色参考设置中的声线支持直接试听预览。

- [新增] H3 分镜切换增加“精准切镜”开关，关闭后无需填写切换时间，生成提示词和拖动分镜轨也不再写入或修改该镜时间戳。

- [优化] 画布提示词中的声音参考标签在文字左侧显示声音图标，便于与图片、视频等引用快速区分。

- [修复] 画布提示词基于完整文本识别跨行长台词，并支持 `(S1) says: <d>…</d>` 等说话人编号与台词标签之间带叙述词的格式，避免台词高亮和说话人徽标消失。

- [修复] 有序组外节点拖到组内成员上时恢复为交换占位，并转移被换出成员的普通输出连线及 H3 Clip 参考绑定；组内互换仍只调整顺序。

- [优化] MCP 画布写操作默认只返回 revision、操作结果和任务摘要，任务查询默认限制为 10 条并改用紧凑 JSON；批量生图支持公共参数以减少重复输入。

- [新增] 增加参考资产高频写入监控，疑似循环写入时记录 Backend 告警并提供按项目查询的诊断接口。

- [优化] H3 参考素材初始化改为单次批量写入，减少打开画布时连续提交 revision 引起的卡顿。

- [修复] H3 提交前统一规范随机种子、LoRA 槽位、模式别名和实际审计值，避免界面显示已选择但 ComfyUI 实际收到默认种子或未启用 LoRA。

- [修复] H3 参考库同步按 Clip 内的绑定独立去重，避免同一素材在不同片段的名称、职责和标签差异引发循环写入及总后台连接误报。

- [优化] H3 分镜提示词的网页与 MCP 入口共用规则清理内部标签和重复切镜描述，并校验引用覆盖、失效素材及切镜时间。

- [优化] H3 分镜提示词生成区分分镜帧与主体参考，按分镜时间同步持续时长和 Ref2VA 任务类型，并保留显式 hard cut。

- [优化] H3 分镜图默认作为每镜入场关键帧，关键帧后保持机位与空间关系并允许人物自然呼吸、转移视线和小幅表演，避免生成只动嘴的静帧视频。

- [调整] H3 节点归入画布元素的“视频”分类，支持当前类型及历史类型。

- [调整] 进入画布时侧边栏默认保持收起，仍可通过顶部按钮手动展开。

- [新增] H3 挂载分镜图时根据该图生成时实际使用的角色参考，自动勾选分镜中出现的人物。

- [修复] 参考资产被多个 H3 Clip 复用时不再因主体字段互相覆盖而持续写入，避免总后台被写入风暴拖慢并误报未启动。

- [修复] 任务中心父任务总进度按子任务平均值单调展示，正式完成前封顶 99%，单个子任务时直接反映其进度。

- [调整] 任务中心右上角运行中角标按父任务聚合，在运行中数量后显示父任务总进度，并在图标底部显示当前运行任务的总体进度条。

- [修复] H3 Output 区域向上拖动时允许压缩 Refs 行到最小高度，避免默认 Refs 布局把 Output 分界线锁死。

- [修复] MCP 创建 H3 节点时统一识别插件名和历史类型别名，正确沿用保存的默认节点尺寸与面板布局。

- [修复] MCP 画布写操作复用最新项目快照预检过期节点与引用，协作冲突不自动重放，并按工具名修正 `canvas_wait_tasks` 延迟统计。

- [调整] 角色节点参考素材编辑隐藏由角色节点固定的职责、提交用途和标签字段。

- [调整] 从节点引用创建新节点时继承原节点当前的画布宽高，并保持新节点以创建位置为中心。

- [修复] H3 南风 V15 适配重装后的 ComfyUI 枚举规则：本地参考素材平铺到 input 根目录，并自动对齐旧画布保存的模型、文本编码器和 VAE 路径。

- [调整] MCP 未显式指定 `projectId` 时跟随浏览器当前聚焦画布；无明确焦点但仅有一个浏览器画布连接时也自动采用该画布，`canvas_set_active_project` 手动指定后按 MCP 会话独立锁定，清空时恢复跟随网页焦点。

- [调整] H3 时间轴左侧的 `Video` 与 `Refs` 标签改为竖向排列，节省标签列横向空间。

- [调整] H3 分镜轨支持不绑定图片的分镜，点击加号直接新建并按规则分配时长，旧图片分镜继续兼容。

- [调整] H3 分镜图在时间轨或参考素材行互换时，分镜与时长随图片一并交换，并避免浮点误差引起无意义的时长重算。

- [修复] H3 分镜轨交换镜头后，分镜编辑器与 `detailed_description` 按新的轨道顺序显示和保存，不再被旧参考图槽位顺序还原。

- [调整] 参考素材职责面板可单独删除分镜图并保留对应分镜。

- [调整] H3 参考素材职责与角色组编辑统一为双击素材打开的弹窗，整合服装/声线设置，并支持放大预览、从画布替换和移除当前素材。

- [新增] H3 分镜编辑器可按 Clip 开启分镜图合并提交，并在提示词与任务提交中标明合成图的镜头格位。

- [优化] 分镜自动生成的 subject_definitions / retention_analysis 不再泄漏参考素材文件名，并按用途细化措辞：首帧图标注为锁定开场帧、尾帧图为锁定收尾帧、分镜图为 approved storyboard frame、视频参考区分运镜编排与剪辑结构、普通音频改为环境声设计参考；缓存版本升级到 11，旧分镜保存时会自动重写这两段。

- [修复] H3 分镜结构化写入和弹框完成动作统一重组六段提示词，清理旧版标准段落前残留的自由文本。

- [优化] H3 节点的实时预览默认单段循环播放；连续播放模式（播放全部）行为不变，播完全部段后正常停止。

- [修复] 打开画布时图片节点按原图比例自动校正尺寸，直接落盘目标尺寸而不切换拖拽预览状态；`useCanvasStore.updateProject` 和 `useCanvasDocument` 对未变化内容短路，避免重复写入触发 React 最大更新深度错误。

- [修复] H3 分镜时长归一化忽略微小浮点误差，避免绑定分镜图后元数据反复写回并触发 React 最大更新深度错误。

- [修复] H3Timeline 的 segments.map(renderStoryboardTrack)/renderRefGrid 输出 React Fragment 缺少 key prop，改为外层显式包 Fragment 并按 segment.id 设 key。

- [优化] 生成日志的输入 refs 缩略图新增放大预览按钮，可在灯箱中查看未被缩略图裁切的完整参考图，与输出缩略图行为一致。

- [优化] 场景色卡提示词字段明确要求按色块列出颜色名称与 HEX 色号，并提供填写示例。

- [优化] 场景色卡可自动提取并编辑栅格图或 SVG 的 HEX 色值，支持多场景有序去重合并并注入图片、视频及 H3 提示词，色卡图仅作预览且提取失败不阻断生成。

- [调整] 放宽工作流名称校验：允许空格、全角符号及生僻字等任意可见字符，仅拦截路径非法字符与控制字符。

- [修复] 画布智能图片节点取消本地 ComfyUI 工作流时同步删除排队任务并中断执行。

- [修复] 智能生成节点可正常选中工作流参数，并按参考图数量记录文生图/图生图模式。

- [调整] 删除自定义工作流时自动解除模型引用，并将原先由该工作流承接的输入场景标记为不支持。

- [修复] 智能生成节点打开图片设置时刷新工作流字段，并在字段 ID 变化后迁移已保存参数，避免生成回退到默认值。

- [修复] 智能图片节点可转换为角色节点或场景节点。

- [修复] 复制画布节点时保留输入参考连线及关联的配置节点连接，副本可继续使用原参考素材。

- [修复] 自定义工作流异步运行后按 AbortSignal 正确轮询 Backend 任务状态，不再把 `{ signal }` 对象误传成 `AbortSignal` 导致浏览器请求失败。

- [修复] Qwen Image 2.1 编辑工作流裁剪时保留 TextEncodeQwenImage21 的可选图片槽位，并按布尔值选择 ComfySwitchNode 分支，单图输入不再误删采样链和输出节点。

- [修复] 多选节点 Ctrl/Cmd+C 将整组选区及内部连线写入画布剪贴板，Ctrl/Cmd+V 可完整粘贴；浏览器拒绝系统剪贴板写入时使用画布内缓存兜底。

- [修复] 在组框或组内成员上按住 Ctrl 拖动会启动增量框选，Ctrl 单击仍按原逻辑切换节点选择。

- [优化] H3 主提示词编辑器按非空换行显示循环主题色，并用同一条加宽的自绘滚动轨道承载滑块与各类结构区块的宽命中跳转刻度，点击时短暂平滑滚动。

- [调整] H3 台词说话人候选菜单显示当前绑定的说话人；当前编号不在角色名册中时仍会作为当前项显示。

- [修复] H3 分镜编辑器与 MCP 写入提示词时不再拼入分镜图标题，只通过绑定引用 ID 使用分镜图；旧提示词会在下次保存时更新。

- [修复] H3 分镜提示词编辑器、时间轨同步与 MCP 写入兼容旧版 `{{ref:...}}` 占位符，并统一归一化为可编辑的 `<Picture N>` 引用标签。

- [调整] H3 分镜提示词中的角色服装引用只写服装描述，不再把服装名称混入主体定义。

- [调整] H3 节点单选按钮字号增至原来的 1.5 倍；下拉框改用 Ant Design `middle` 尺寸，当前值使用 28px 字号并留足行高，避免文字被边框裁切。

- [修复] H3 下拉框通过 Select 语义样式和 `labelRender` 直接设置当前值字号，避免只放大下拉选项而收起后的值仍然偏小。

- [修复] H3 分镜编辑器载入时循环剥离正文中残留的切镜自然句与分镜图 cue（旧版解析曾把这对生成句泄进可编辑正文，反复保存会继续叠加），分镜时间轨同步同样自愈；分镜图下拉框不再出现「分镜图·分镜图·」叠前缀。

- [调整] H3 主提示词富文本编辑器字号增至原来的约 1.3 倍，长提示词更易阅读。

- [修复] H3 分镜提示词编辑弹窗恢复主题变量传递，各区域重新显示边框与底色并拉开间距，不再糊成一团。

- [修复] 开启 H3 分镜轨后按引用顺序自动绑定分镜图与提示词镜头，分镜编辑器无需逐镜手动选择。

- [调整] H3 分镜提示词切镜点统一序列化为 `MM:SS.mmm`，转场使用可回读的自然句。

- [新增] Ref2VA H3 的每个 Clip 独立提供分镜时间轨，支持按时长显示分镜、拖动调时与排序，并同步维护该 Clip 的提示词分镜区块。

- [新增] H3 角色编辑弹窗（服装与声线）的每套服装缩略图右上角新增放大预览按钮，可在灯箱中查看未被缩略图裁切的完整服装图。

- [调整] 角色参考图的主要职责改由角色节点逐图设置，H3 参考弹窗锁定角色图职责和普通参考用途。

- [修复] H3 时间线上把角色组引用拖到其他 Clip 时保留角色组、服装选择和声线状态，不再降级为独立图片。

- [修复] H3 空参考槽点选角色节点时在鼠标按下捕获阶段完成选择，并支持仅有存储键的角色参考图，避免节点内按钮吞掉选择或角色被误判为不可选。

- [优化] 画布压缩包导入、单项目/批量导出及当前画布导出期间显示 loading 并互斥，避免长耗时任务被重复触发。

- [调整] MiniMax H3「分镜编辑」入口仅在 Ref2VA 多参考模式开放，避免其他模式出现不适用的结构化分镜编辑。

- [修复] H3 角色组随源角色节点的服装与声线变更同步目录和派生引用，避免引用校验或生成使用过期素材。

- [修复] H3 生成中的实时预览按父任务 ID 正确显示，并在任务切换时清除旧预览。

- [修复] ComfyUI WebSocket 的逐节点 `executed` 事件不再被误判为整图完成，避免长时间运行的 H3 任务提前报“history 与 WebSocket 输出均不可用”。

- [修复] H3 精修不再把 `infinite-canvas-cache/output` 中作为输入的一采视频误认成二采结果。

- [修复] H3 确认人脸精修改为由 Infinite Canvas 自有节点对缓存一采裁脸重采样并贴回，Clip 原图片参考按原序用于局部 H3 条件，不再依赖 Director 运行时，避免整段视频重生成造成镜头和动作漂移。

- [修复] H3 一采确认不再因参考资产重复登记刷新时间戳而误报生成参数或上游已变化。

- [修复] H3 确认精修校验按旧版一采指纹的原始参数与参考快照兼容，避免未改生成内容却被要求重生成。

- [优化] 图片节点宽度不足 200px 时，顶部「下载 / 创建副本 / 设为主图」按钮自动只显示图标（悬停仍有文字提示），右上角「N 张」按钮同步缩成只显示张数的圆形按钮，不再互相压住。

- [新增] H3 提示词区可打开结构化分镜编辑弹框，支持填写 summary、分镜前的总体描述、分镜、声景和配乐，并在镜头间设置切换时间；总体描述可用 @ 插入人物，镜头描述可用 @ 插入当前 Clip 引用的分镜图或人物。

- [调整] H3 分镜提示词仅在点击「完成」时统一保存；关闭弹框会丢弃未提交修改，提交加载状态仅在用户点击「完成」后显示。

- [调整] H3 分镜图职责弹窗移除标签和引用程度项；改在结构化分镜中按镜头选择图片并设置引用程度，所选状态写入 Picture 的 `retention_analysis`。

- [调整] H3 分镜切换方式统一为连续镜头（不切镜）、硬切、叠化和淡出至黑场再淡入；第一镜忽略转场，后续分镜转场按上一镜到当前镜生成，分镜图只作为目标构图参考。

- [调整] H3 分镜提示词按引用库已有的画面摘要、标签和素材名生成分镜图描述，并按选择的转场生成连接句；提示词拼接不调用模型，引用分析摘要可通过素材弹窗的手动分析保存。

- [修复] H3 结构化分镜描述的 @ 候选增加当前 Clip 引用的分镜图，并解析已有的分镜图引用标记，避免显示“素材未找到”。

- [新增] Ref2VA 分镜编辑完成后按规则生成主体、图片、音频引用定义与保留分析，覆盖人物声线并以 SHA-256 指纹缓存未变内容，不调用文本模型。

- [修复] Ref2VA 自动生成区分主体身份定义与素材保留结算；仅把实际用于镜头锚点的图片单列为 Picture，并避免将动作或站位误写为人物保真要求。

- [调整] Ref2VA 自动生成的 `subject_definitions` 与 `retention_analysis` 将图片引用保留为绑定标记，在提示词编辑器中显示对应缩略图，提交时再编译为 `<Picture N>`；升级缓存版本以刷新旧的纯文本引用。

- [调整] MCP 结构化 H3 分镜写入对齐 UI 的转场描述、分镜图引用、主体与保留分析规则及 v10 SHA-256 缓存指纹；相同分镜输入可跨 UI 和 MCP 命中缓存。

- [调整] 收紧 H3 分镜编辑弹框中的标签、输入框与分镜正文字号，降低密集提示词的阅读负担。

- [修复] H3 提示词转换将 Subject 与 Picture 独立编号，并把人物主体定义绑定到对应人物参考图，修正已有提示词中的错位编号。

- [调整] H3 提示词 @ 候选来自当前 Clip 的引用素材与出镜人物；未匹配的人物或素材标记可点击并从当前候选中重新绑定。

- [修复] 图片等节点顶部工具条（下载 / 创建副本 / 张数等）双击不再误触发放大预览或编辑：节点双击处理原先只看节点类型和内容、不看双击落点，而按钮 `onClick` 上的 `stopPropagation` 拦不住**独立冒泡的 `dblclick` 事件**；现在节点级双击会跳过落在按钮与输入控件上的事件，工具条容器本身也吞掉双击。双击图片本体弹预览的行为不变。

- [修复] MiniMax H3 提交生成时自动按角色组修正派生服装绑定的 `subjectId`，避免旧绑定数据阻断预检和生成。

- [新增] H3 节点 ref2va 模式的参考插槽不再有数量上限，可不断新增；超出运行时限制（图片 9 / 视频 3 / 音频 3）的插槽会标红提示。

- [调整] H3 节点点击空白参考插槽改用画布已有的「选节点作参考」交互：目标节点高亮、其余可作参考的节点显示可选态、顶栏提示「从画布选择参考 · ESC 返回输入框」，点选即填入参考；选角色节点时自动展开为人物形象与音色参考，且不新增连线。

- [调整] MiniMax H3 提示词统一使用 `<Subject N>`、`<Picture N>`、`<Video N>` 和 `<Audio N>` 标签，编辑器输入或插入后自动显示对应人物或素材缩略图。

- [优化] MiniMax H3 工作台的失败提示可点击复制完整错误信息，便于排查参考绑定等校验失败。

- [调整] 画布元素按图片、视频、文本、音频筛选时，同时展示对应生成模式的智能节点；智能图片节点显示当前主图缩略图。

- [新增] 智能图片节点保留跨次生成的图片历史，展开后可选主图并恢复对应提示词、模型参数和参考图。

- [修复] 智能图片节点默认沿实时连线读取上游参考图；只有用户主动选中历史版本时，才继续使用该版本保存的参考图快照。

- [修复] H3 Clip 已引用的智能图片节点重新生成后，参考缩略图、绑定和下次生成输入会同步到来源主图。

- [修复] H3 节点手动拖好的各模块宽高在刷新浏览器后不再被还原：工作台视图状态（各模块宽高、当前片段、播放指针）按节点写入浏览器本地缓存，刷新后原样恢复。

- [修复] H3 节点刷新浏览器（或节点重新挂载）后视频不再自动播放：只有用户真正点了播放才起播。

- [调整] H3 节点自身还没有任何布局值时，改用「设为默认参数」保存的默认布局，不再回落到内置的 220/960/480/320/150。
...
- [修复] 角色节点恢复始终显示 `characterPrimaryIndex` 指定的主图，不再被引用它的生成节点改写封面：此前按「全画布第一个引用者」显示所选参考图，导致主图被无关分镜的服装图顶掉（如沈昭宁被分镜01 选中的四视图覆盖身份图）。
...
- [调整] H3 时间轴横向滚动条与主提示词竖向滚动条去掉「跳转到 Clip / 区块」的点按标记，回到纯滚动条：按下即让滑块中心对齐指针并支持按住连续拖动，不再有 180ms 缓动，点到哪滚到哪；两条轨道加宽便于命中。
