...
- [优化] Output 卡片随面板高度自适应放大：卡片高度此前固定 78px，拉高 Output 区域只剩更多空白。现用 ResizeObserver 实测 Output 列表可用高度，按 1–3 行均分出卡片高度（clamp 78–380px，经 CSS 变量 `--h3-out-card-h` 下发），列宽最小值同步缩放保持 2:1 视觉比例；时间轴/Output 手柄拖动时卡片即时跟随。
