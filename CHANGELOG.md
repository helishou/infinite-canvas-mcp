...
- [优化] H3 主提示词编辑器按非空换行显示循环主题色，并在加宽的滚动条轨道内按完整提示词位置稳定显示各类 H3 结构区块的快速跳转刻度。
...
- [调整] MiniMax H3 提示词编辑器把 `{{subject:…}}` 显示为人物节点引用块、把 `{{ref:…}}` 显示为素材缩略图，支持用 @ 选择并整块删除；人物标记按生成端规则解析绑定与项目参考库中的主体 ID，分镜出镜人物也可识别，未匹配标记可点击后从当前 Clip 候选重新绑定。
